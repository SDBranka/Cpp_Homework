...
void userNumber(int &);				  //allows player to input a number
int computerGuess(int &);			  //generates a computer "guess"
...
int main()
{
	...
	do
	{
		userNumber(userNum); //allows player to input a number
		cout << "Computer guesses: ";
		do
		{
			cout << computerGuess(comRandNum) << "   "; //generates a computer "guess"
			if (scoreTotal % 7 == 0)
				cout << "\n                  ";
			check = checkGuess(comRandNum, userNum, scoreTotal); //compares the two numbers and returns a score value
			scoreTrack(scoreTotal);								 //keeps track of the score
		} while (check == 0);
		alterStats(gameCount, scoreTotal, bestScore); //modifies and monitors game stats
...
void userNumber(int &uNumber) //allows player to input a number
{
	cout << "\nEnter a target value: ";
	cin >> uNumber;
}

int computerGuess(int &comGuess) //generates a computer "guess"
{
	comGuess = 1 + (rand() % 100);
